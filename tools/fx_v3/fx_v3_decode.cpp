// FX-V3: integer twin of the MERGED Hebbian-MoE crystal (FX-V2 core
// + the switch_top1 router gate). Pure int64, no libm, no torch. The
// gate (spec in relay 2026-07-31-3): router logits via the standard
// sigma-law gemm (a-scale), s = rdiv(rl*Q, A) to Q units, house
// detbwd_r1b exp table (Q=512, domain [-8,0]), integer max-shift,
// top_p = rdiv(rexp[-1]*Q, sum(e)), FFN output scaled by
// rdiv(down_out * top_p, Q) before the residual clamp.
// Emits one greedy stream per prompt row + the streams digest
// (Python repr convention, as FX-V2).
//
// Build: c++ -O2 -std=c++17 fx_v3_decode.cpp -o fx_v3_decode
// Run:   ./fx_v3_decode fx3_tables.bin prompts.txt 40
#include <cstdint>
#include <cstdio>
#include <cstring>
#include <fstream>
...
static const i64 Q = 512, TSE = 8 * 512;  // router softmax (detbwd_r1b)
...
struct DetLM {
  ...
  std::vector<i64> step(int tok, int pos) {
    ...
    for (int l = 0; l < LAYERS; l++) {
      ...
      std::vector<i64> rl = gemm(h, p + ".router");
      const Tensor& RX = t.at("rexp.tab");
      i64 smax = INT64_MIN;
      std::vector<i64> sq(rl.size());
      for (size_t e = 0; e < rl.size(); e++) {
        sq[e] = rdiv(rl[e] * Q, A);
        if (sq[e] > smax) smax = sq[e];
      }
      i64 z = 0;
      for (size_t e = 0; e < rl.size(); e++) {
        i64 dd = sq[e] - smax;
        z += dd < -TSE ? 0 : RX.data[dd + TSE];
      }
      i64 top_p = rdiv(RX.data[TSE] * Q, z < 1 ? 1 : z);
      for (int i = 0; i < D; i++)
        x[i] = clampi(x[i] + rdiv(dwn[i] * top_p, Q), -ACT_CLAMP, ACT_CLAMP);
    }
    x = rmsnorm(x, "norm");
    return gemm(x, "head");
  }
...
int main(int argc, char** argv) {
  if (argc != 4) {
    fprintf(stderr, "usage: %s tables.bin prompts.txt n_new\n", argv[0]);
    return 1;
  }
  ...
  for (size_t s = 0; s < battery.size(); s++) {
    ...
    for (size_t i = 0; i < toks.size(); i++) {
      ln += std::to_string(toks[i]);
      rep += std::to_string(toks[i]);
      if (i + 1 < toks.size()) { ln += " "; rep += ", "; }
    }
    rep += "]";
  ...
  fprintf(stderr, "FX-V3 streams (c++): %s\n", hs.hex().c_str());
  return 0;
}
