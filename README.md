...
| `ax::nn`   | Exact NN: AXNN container format (fp32 and fixed-point), reference fp32 inference (`model`), integer-only fixed-point inference (`exact_model`, Q.16 operands / int64 accumulation / container-shipped transcendental tables), and the `intbirth` integer-training engine (`block`, `adamw`, `full_birth`, `multi_birth`, `moe_birth`) with a pybind11 bridge; ENGINE-EXACT-1 precision ladder (Q9 shipped / Q32 / Q64 rungs, identical rounding placement) plus an exact-rational prefix anchor with zero value-rounding |
