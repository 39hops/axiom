...
constexpr std::array<std::uint64_t, 80> kK = {
...
std::array<std::uint8_t, 64> sha512(std::span<const std::uint8_t> data) {
  ...
  for (int i = 0; i < 8; ++i) msg.push_back(0);  // high 64 bits (sizes < 2^64)
  for (int i = 7; i >= 0; --i)
    msg.push_back(static_cast<std::uint8_t>(bitlen >> (8 * i)));
  ...
  for (std::size_t off = 0; off < msg.size(); off += 128) {
    for (int t = 0; t < 16; ++t) {
      std::uint64_t v = 0;
      for (int b = 0; b < 8; ++b)
        v = (v << 8) | msg[off + 8 * static_cast<std::size_t>(t) +
                           static_cast<std::size_t>(b)];
      w[static_cast<std::size_t>(t)] = v;
    }
    for (int t = 16; t < 80; ++t) {
      const std::uint64_t s0 = rotr(w[t - 15], 1) ^ rotr(w[t - 15], 8) ^
                               (w[t - 15] >> 7);
      const std::uint64_t s1 = rotr(w[t - 2], 19) ^ rotr(w[t - 2], 61) ^
                               (w[t - 2] >> 6);
    ...
    for (int t = 0; t < 80; ++t) {
      ...
      const std::uint64_t t1 =
          hh + S1 + ch + kK[static_cast<std::size_t>(t)] +
  ...
  for (int i = 0; i < 8; ++i)
    for (int b = 0; b < 8; ++b)
      out[static_cast<std::size_t>(8 * i + b)] =
